...
module multicycle_rv32i (
    ...
    // --- Interrupts & CSR Wires ---
    wire        ext_irq; // NAYA: Ab ye internal wire hai jo UART se CSR tak jayegi
    wire        ecc_double_err_irq; //for ecc error
    wire        trap_pending, trap_entry, trap_exit, csr_write;
    wire [31:0] mtvec_out, mepc_out, csr_read_data;
    ...
    wire        ecc_status_re, ecc_status_we;      // <-- NAYA
    wire [31:0] ecc_status_addr, ecc_status_rdata; // <-- NAYA

    // --- ECC status signals (dmem wrapper se) ---
    wire        sb_sticky, db_sticky;              // <-- NAYA
    wire [31:0] sb_count, db_count;                // <-- NAYA
    ...
    always @(posedge clk) begin
        ...
        b_reg   <= read_data2;
        alu_out <= alu_result;
        csr_old_val <= csr_read_data;   // <-- NAYA:
    end
    ...
    wire is_system_instr = (ir[6:0] == 7'b1110011);
    assign write_back_data = (is_system_instr) ? csr_old_val : 
                             (mem_to_reg)      ? mdr : alu_out;

    // Naya wire jo sirf DATA memory access ke liye true ho, instruction fetch ke liye nahi
    wire data_mem_read = mem_read & i_or_d;                         
    ...
    soc_interconnect bus_inst (
        ...
        .ecc_status_re(ecc_status_re),        // <-- NAYA
        .ecc_status_we(ecc_status_we),        // <-- NAYA
        .ecc_status_addr(ecc_status_addr),    // <-- NAYA
        .ecc_status_rdata(ecc_status_rdata)   // <-- NAYA
    );
    ...
    /*data_mem dmem_inst (
        ...
        .read_data(ram_rdata)
	     ...
	     // Data Memory (ab ECC-protected, but same interface)
    ecc_dmem_wrapper dmem_inst (
        .clk(clk),
        .rst(rst),
        .mem_read(ram_re),       
        .mem_write(ram_we),      
        ...
        .write_data(ram_wdata),
        .read_data(ram_rdata),
        .double_err_irq(ecc_double_err_irq),
        .sb_sticky_o(sb_sticky),      // <-- NAYA
        .db_sticky_o(db_sticky),      // <-- NAYA
        .sb_count_o(sb_count),        // <-- NAYA
        .db_count_o(db_count),         // <-- NAYA
		  .fi_en(fi_en),           // <-- NAYA
		  .fi_mask(fi_mask)        // <-- NAYA
    );
    ...
    csr_file csr_inst (
        ...
        .trap_exit(trap_exit),
        .current_pc(old_pc),
        .ecc_double_err_irq(ecc_double_err_irq),
        .mtvec_out(mtvec_out),
        .mepc_out(mepc_out),
        .trap_pending(trap_pending)
    );
    ...
    // ECC Status Registers (memory-mapped, 0x5000_0000 range)
    ecc_status_regs ecc_status_inst (
        .clk        (clk),
        .re         (ecc_status_re),
        .we         (ecc_status_we),
        .addr       (ecc_status_addr),
        .sb_sticky  (sb_sticky),
        .db_sticky  (db_sticky),
        .sb_count   (sb_count),
        .db_count   (db_count),
        .rdata      (ecc_status_rdata),
        .clear_pulse()   // abhi use nahi kar rahe, unconnected chhor do
    );
