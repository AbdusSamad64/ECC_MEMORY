...
module control_unit (
    ...
    output reg        ir_write,
	 output reg        old_pc_write,   // <-- NAYA SIGNAL
    output reg        pc_write,
    output reg        reg_write,
    output reg        mem_write,
    output reg        i_or_d,
    ...
    // FSM States (purani values 0-6 bilkul same rakhi hain)
    localparam FETCH     = 4'b0000;
    localparam DECODE    = 4'b0001;
    localparam EXECUTE   = 4'b0010;
    localparam MEMORY    = 4'b0011;
    localparam WRITEBACK = 4'b0100;
    localparam BRANCH    = 4'b0101;
    localparam TRAP      = 4'b0110;
    // Naye wait states (sirf inhi 2 encodings ka izafa hua hai)
    localparam FETCH2    = 4'b0111; // imem sync-read latency wait
    localparam MEMORY2   = 4'b1000; // dmem sync-read latency wait
    ...
    always @(*) begin
        case (state)
            FETCH:   next_state = FETCH2; // pehle FETCH->DECODE tha

            FETCH2:  next_state = DECODE; // naya: ek cycle wait, ab ir_write yahan hoga
            ...
            MEMORY: begin
                if (opcode == 7'b0000011)      // LOAD -> ek extra wait cycle chahiye
                    next_state = MEMORY2;
                else                            // STORE -> write already sync, wait ki zaroorat nahi
                    next_state = (trap_pending) ? TRAP : FETCH;
            end

            MEMORY2: next_state = WRITEBACK;   // naya: yahan dmem ka data ready hoga
    ...
    always @(*) begin
        ...
        case (state)
            FETCH: begin
                i_or_d    = 1'b0;
					 old_pc_write = 1'b1;   // <-- YAHAN CAPTURE
                mem_read  = 1'b1;      // address (pc) is drive karo, sync mem is cycle capture karega
                alu_src_a = 2'b00;
                alu_src_b = 2'b01;
                alu_ctrl  = 4'b0000;
                pc_write  = 1'b1;
                pc_source = 1'b0;
                // ir_write yahan se hata diya
            end

            FETCH2: begin
                // Is cycle mein inst_mem ka registered output valid ho chuka hai
                ir_write = 1'b1;      // ab ir load karo
            end
            ...
            MEMORY: begin
                i_or_d = 1'b1;
					 ...
					 alu_src_b = 2'b10;   // imm
                if (opcode == 7'b0000011) mem_read  = 1'b1;  // address drive, sync mem yahan capture karega
                if (opcode == 7'b0100011) mem_write = 1'b1;  // store abhi bhi sync, ek hi cycle
            end

            MEMORY2: begin
                // Idle wait: dmem ka registered read_data ab is cycle mein valid hai
                // (top module ka "mdr <= data_mem_out" block isi cycle ke posedge par ise pakdega)
                i_or_d = 1'b1;
					 alu_src_a = 2'b01;   // a_reg
					 alu_src_b = 2'b10;   // imm
					 mem_read = 1'b1;   // <-- YEH ADD
            end
